...
describe('Get Reviews', () => {
  it('Should return valid values', () => frisby
    ...
    .expect('status', 200)
    .expect('bodyContains', 'review_id', 'rating', 'summary', 'recommend', 'response', 'body', 'date', 'reviewer_name', 'reported', 'helpfulness', 'photos'));

  it('Should return error with extra column', () => frisby
    .get(path.join(baseUrl, '/reviews?product_id=37311'))
    .expect('status', 200)
    .expect('bodyContains', 'id', 'review_id', 'rating', 'summary', 'recommend', 'response', 'body', 'date', 'reviewer_name', 'reported', 'helpfulness', 'photos'));
});
...
describe('Mark Helpful and Mark Reported', () => {
  it('Should return status 200 when marked helpful', () => frisby
    .put(path.join(baseUrl, '/reviews/1/helpful'))
    .expect('status', 200));
  it('Should return status 200 when marked reported', () => frisby
    .put(path.join(baseUrl, '/reviews/1/report'))
    .expect('status', 200));
});

describe('Post reviews', () => {
  it('Should return status 201 when successful posted a review', () => frisby
